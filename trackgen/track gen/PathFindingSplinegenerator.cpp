...
void pf::generate(Spline& build_path, bool& exitThread, int& thread_finished)
{
	try
	{
		...
		auto filter = [&]() -> int
		{
			...
			auto sub3 = [&]() -> bool
			{
				for (size_t i = 0; i < build_path.size() - 3; i++)
				{
					for (size_t j = i + 1; j < build_path.size() - 1; j++)
					{
						if(build_path.segmentsCollide(i, j));
						{
							auto [_1, a, b, _2] = build_path.getSegment(i);
							auto [_3, c, d, _4] = build_path.getSegment(j);
							auto ap = castOnVec(c, d, a);
							auto bp = castOnVec(c, d, b);
							auto the_chosen_one = ap;
							auto the_original_one = a;
							if (distance(ap, a) > distance(bp, b))
							{
								the_chosen_one = bp;
								the_original_one = b;
							}
							auto dir = b - bp;
							auto norm_dir = direction(sf::Vector2f(dir.y, dir.x)) * -1.f;
			...
			while (true)
			{
				...
				if (!sub2()) continue;
				if (!sub3()) continue;
				if (build_path.size() < 10) return 2;
