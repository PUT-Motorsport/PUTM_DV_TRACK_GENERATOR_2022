...
#define PRED_RETURN_BOOL(type, conditon)	\
[&](type)->bool								\
{											\
	return conditon;						\
}

#define PRED_CMD_CHECK_2(val1, val2) PRED_RETURN_BOOL(std::string arg, arg == val1 || arg == val2)
...
int mainGUI()
{
    ...
    if (Config::initConfig() == 0) return 0;
...
int mainProccess(int argc, char* argv[])
{
	...
	if (std::find_if(args.begin(), args.end(), PRED_CMD_CHECK_2("-sp", "--sub-process")) == args.end())
	{
		return 0;
	}
	...
	while (true)
	{
		...
		auto cmds = split(cmd, ';');
		if (std::find_if(cmds.begin(), cmds.end(), PRED_CMD_CHECK_2("-S", "--Stop")) != cmds.end())
		{
			return 0;
		}
		else if (std::find_if(cmds.begin(), cmds.end(), PRED_CMD_CHECK_2("-s", "--start")) != cmds.end())
		{
			std::unique_ptr < ISplineGenerator > spline_gen(new PathFindingSplineGenerator());
			spline_gen->generateFullTrack();
			std::unique_ptr < IConeGenerator > cone_gen(new UniformConeGenerator(spline_gen->getTrack()));
			cone_gen->generateAllCones();
			auto cones = cone_gen->getCones();
			...
			for (auto& cone : cones)
			{
				std::cout << cone.pos.x << " " << -cone.pos.y << " " << (int)cone.type << "\n";
			}
		}
	}
...
std::vector < std::string > split(std::string str, char c)
{
	...
	{
		std::string out;
		std::getline(ss, out, c);
		split_str.push_back(out);
	}
	std::erase_if(split_str, [](std::string s)->bool
		{
			return s.empty();
		});
	return split_str;
	//std::erase(std::remove_if(split_str.begin(), split_str.end(), ), split_str.end());
}

int main(int argc, char* argv[])
{
#if _DEBUG
    auto buff2 = std::string("-sp");
    auto buff = (char*)buff2.c_str();
    argv[0] = buff;
#endif
    if (argc == 1) mainGUI();
    else mainProccess(argc, argv);
